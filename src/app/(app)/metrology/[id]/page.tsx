...
export default function MetrologyToolDetailPage() {
    ...
    const fetchToolData = async () => {
      ...
      try {
        ...
        if (!fetchedTool) {
          ...
          try {
            const logs = await metrologyService.getCalibrationLogs(fetchedTool.id);
  ...
  const getStatusBadgeVariant = (status: MetrologyTool['status']) => {
    switch (status) {
      case 'calibrated': return 'default'; // Often green or blue
      case 'due_calibration': return 'secondary'; // Check theme for yellow/orange
      case 'out_of_service': return 'destructive';
      case 'awaiting_calibration': return 'outline'; // Check theme for a neutral/pending color
        ...
        <div className="lg:col-span-2 space-y-6">
          <Card>
            ...
            <CardContent className="space-y-4">
              <div className="grid grid-cols-2 gap-4 text-sm">
                ...
                <div><strong>Serial Number:</strong> {tool.serialNumber}</div>
                <div><strong>Manufacturer:</strong> {tool.manufacturer || 'N/A'}</div>
                <div><strong>Location:</strong> {tool.location || 'N/A'}</div>
                <div>
                  <strong>Status:</strong> <Badge variant={getStatusBadgeVariant(tool.status)} className="capitalize">{tool.status.replace(/_/g, ' ')}</Badge>
                </div>
                <div><strong>Calibration Interval:</strong> {tool.calibrationIntervalDays} days</div>
                {tool.lastCalibrationDate && <div><strong>Last Calibrated:</strong> {new Date(tool.lastCalibrationDate).toLocaleDateString()}</div>}
                {tool.nextCalibrationDate && <div><strong>Next Calibration Due:</strong> <span className={new Date(tool.nextCalibrationDate) < new Date() && tool.status !== 'calibrated' ? 'text-destructive font-bold' : ''}>{new Date(tool.nextCalibrationDate).toLocaleDateString()}</span></div>}
              </div>
              {tool.notes && (
                <>
            ...
            <CardFooter>
                 <Button>
                    <CalendarPlus className="mr-2 h-4 w-4" /> Log New Calibration
                </Button>
            </CardFooter>
          </Card>
          ...
          <Card>
            <CardHeader>
              ...
              <CardDescription>Recent calibration logs for this tool.</CardDescription>
            </CardHeader>
            <CardContent>
              {calibrationLogs.length > 0 ? (
                  ...
                  <TableBody>
                    {calibrationLogs.map(log => (
                      <TableRow key={log.id}>
                        ...
                        <TableCell><Badge variant={getResultBadgeVariant(log.result)} className="capitalize">{log.result}</Badge></TableCell>
                        <TableCell className="hidden sm:table-cell">{log.nextDueDate ? new Date(log.nextDueDate).toLocaleDateString() : 'N/A'}</TableCell>
                        <TableCell>
                          <Button variant="link" size="sm" asChild>
                            <Link href={log.certificateUrl || '#'} target={log.certificateUrl ? "_blank" : "_self"}>
                              {log.certificateUrl ? "View Cert" : "Details"}
                            </Link>
                          </Button>
                        </TableCell>
                      </TableRow>
                ...
                <p className="text-muted-foreground text-sm">No calibration logs found for this tool.</p>
              )}
            </CardContent>
          </Card>
        </div>
