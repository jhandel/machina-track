...
export default function MetrologyPage() {
  ...
  const [filter, setFilter] = useState<
    "all" | "calibrated" | "due_calibration" | "overdue" | "out_of_calibration"
  >("all");
    ...
    try {
      ...
      switch (filter) {
        ...
        case "out_of_calibration":
          fetchedTools = await metrologyService.getByStatus(
            "out_of_calibration"
          );
          break;
        default:
    ...
    <div>
      ...
      <div className="mb-6 flex flex-col sm:flex-row gap-4">
        ...
        <div className="flex gap-2">
          ...
          <Button
            variant={filter === "out_of_calibration" ? "default" : "outline"}
            size="sm"
            onClick={() => setFilter("out_of_calibration")}
          >
            Out of Calibration
          </Button>
        </div>
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            {filter === "all"
              ? "All metrology tools in the system"
              : filter === "calibrated"
              ? "Tools with valid calibration"
              : filter === "due_calibration" || filter === "overdue"
              ? "Tools requiring calibration attention"
              : "Tools currently out of calibration"}
          </CardDescription>
        </CardHeader>
        <CardContent>
              ...
              {filteredTools.map((tool) => (
                ...
                >
                    ...
                    <div className="flex-1">
                      <div className="flex items-center gap-2 mb-2">
                        ...
                        <Badge
                          ...
                          variant="secondary"
                        >
                          {tool.status.replace(/_/g, " ").toUpperCase()}
                        </Badge>
                      </div>
                      <div className="text-sm text-muted-foreground space-y-1">
                        <div className="flex items-center gap-4">
                          ...
                          {tool.manufacturer && (
                            <span>
                              <strong>Mfg:</strong> {tool.manufacturer}
                            </span>
                          )}
                        </div>
                        {tool.location && (
                  ...
                  <div className="mt-3 flex gap-2">
                    ...
                    {(tool.status === "due_calibration" ||
                      tool.status === "awaiting_calibration") && (
                      <Button
                        variant="outline"
                        size="sm"
                        onClick={() => {
