      ...
      try {
        ...
        if (!fetchedTask) {
          ...
          try {
            ...
            const fetchedEquipment = await equipmentService.getById(fetchedTask.equipmentId);
          ...
          try {
            ...
            const records = await serviceRecordService.getByTaskId(fetchedTask.id);
  ...
  return (
    <div>
      <PageHeader 
        ...
        icon={ClipboardList}
        description={`Maintenance for: ${equipment ? `${equipment.name} (${equipment.model})` : 'Equipment'}`}
        actions={
        ...
        <div className="lg:col-span-2 space-y-6">
          <Card>
            ...
            <CardContent className="space-y-4">
              <div className="grid grid-cols-2 gap-4 text-sm">
                <div><strong>Equipment:</strong> {equipment ? `${equipment.name} (${equipment.model})` : 'Loading...'}</div>
                <div><strong>Serial Number:</strong> {equipment?.serialNumber || 'N/A'}</div>
                <div><strong>Location:</strong> {equipment?.location || 'N/A'}</div>
                <div>
                  <strong>Status:</strong> <Badge variant={getStatusBadgeVariant(task.status)} className="capitalize">{task.status.replace(/_/g, ' ')}</Badge>
                </div>
                {task.nextDueDate && <div><strong>Due Date:</strong> <span className={new Date(task.nextDueDate) < new Date() && task.status !== 'completed' ? 'text-destructive font-bold' : ''}>{new Date(task.nextDueDate).toLocaleDateString()}</span></div>}
                {task.lastPerformedDate && <div><strong>Last Performed:</strong> {new Date(task.lastPerformedDate).toLocaleDateString()}</div>}
                {task.frequencyDays && <div><strong>Frequency:</strong> Every {task.frequencyDays} days</div>}
                {task.assignedTo && <div><strong>Assigned To:</strong> {task.assignedTo}</div>}
              </div>
              {task.notes && (
                <>
                  <Separator className="my-4" />
                  <div>
                    <h4 className="font-semibold mb-1">Notes / Instructions:</h4>
                  ...
                  <div>
                    ...
                    <ul className="list-disc pl-5 text-sm text-muted-foreground">
                      {task.partsUsed.map((part, index) => (
                        <li key={index}>{part.partName} (Quantity: {part.quantity})</li>
                      ))}
                    </ul>
                  </div>
                </>
            ...
            <CardFooter>
              {task.status !== 'completed' && task.status !== 'skipped' ? (
                <Button onClick={handleMarkCompleted}>
                  <CheckSquare className="mr-2 h-4 w-4" /> Mark as Complete
                </Button>
              ) : (
          ...
          <Card>
            <CardHeader>
              ...
              <CardDescription>Service records for this maintenance task.</CardDescription>
            </CardHeader>
            <CardContent>
              {serviceRecords.length > 0 ? (
                <Table>
                  <TableHeader>
                    <TableRow>
                      ...
                      <TableHead className="hidden sm:table-cell">Work Description</TableHead>
                  ...
                  <TableBody>
                    {serviceRecords.map(record => (
                      <TableRow key={record.id}>
                        ...
                        <TableCell className="hidden sm:table-cell">{record.descriptionOfWork}</TableCell>
                        <TableCell className="hidden sm:table-cell">{record.cost ? `$${record.cost.toFixed(2)}` : 'N/A'}</TableCell>
                        <TableCell>
                          <Button variant="link" size="sm" asChild>
                            <Link href={`/service-records/${record.id}`} className="inline-flex items-center justify-center gap-2">
                              View Details
                            </Link>
                          </Button>
                        </TableCell>
                ...
                <p className="text-muted-foreground text-sm">No service records found for this task.</p>
              )}
            </CardContent>
          </Card>
        </div>
        ...
        <div className="space-y-6">
          <Card>
            ...
            <CardContent className="flex flex-col gap-2">
              <Button variant="outline" asChild>
                <Link href={`/service-records/new?taskId=${task.id}`} className="inline-flex items-center justify-center gap-2">
                  <History className="mr-2 h-4 w-4" /> Log Service Record
                </Link>
              </Button>
              {task.status !== 'completed' && task.status !== 'skipped' ? (
            ...
            <CardContent className="flex flex-col gap-2">
              {equipment ? (
                <Button variant="link" asChild className="justify-start px-0">
                  <Link href={`/equipment/${equipment.id}`} className="inline-flex items-center justify-center gap-2">
                    View Equipment Details
                  </Link>
                </Button>
              ) : (
                <Button variant="link" asChild disabled className="justify-start px-0">
                  View Equipment Details
                </Button>
              )}
              <Button variant="link" asChild className="justify-start px-0">
